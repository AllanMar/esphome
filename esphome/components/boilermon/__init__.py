...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Required(CONF_MODE_PIN): pins.internal_gpio_output_pin_schema,
...
async def to_code(config):
    ...
    pin = await cg.gpio_pin_expression(config[CONF_MODE_PIN])
    cg.add(var.set_mode_pin(pin))
    pin = await cg.gpio_pin_expression(config[CONF_STEP_PIN])
    cg.add(var.set_step_pin(pin))
